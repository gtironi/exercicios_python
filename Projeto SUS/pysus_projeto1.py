...
agrupado_por_municipio = dataframe_lepto_2018.groupby('ID_MUNICIP').count().sort_values('TP_NOT', ascending=False)
# print(agrupado_por_municipio['TP_NOT'])

# Estado

agrupado_por_estado = dataframe_lepto_2018.groupby('SG_UF').count().sort_values('TP_NOT', ascending=False)
...
agrupado_por_idade = dataframe_lepto_2018.groupby('idade').count().sort_values('TP_NOT', ascending=False)
# print(agrupado_por_idade['TP_NOT'])

# Sexo
agrupado_por_sexo = dataframe_lepto_2018.groupby('CS_SEXO').count().sort_values('TP_NOT', ascending=False)
# print(agrupado_por_sexo['TP_NOT'])

# Raça e cor
agrupado_por_raca = dataframe_lepto_2018.groupby('CS_RACA').count().sort_values('TP_NOT', ascending=False)
